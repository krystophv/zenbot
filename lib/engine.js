...
module.exports = function (s, conf) {
  ...
  function initBuffer (trade) {
    ...
    var de = tb(trade.time).resize(so.period_length).add(1)
    s.period = {
  ...
  function placeOrder (type, opts, cb) {
    ...
    if (so.mode !== 'live') {
      if (!order.orig_time) order.orig_time =  s.period.latest_trade_time || s.period.close_time
      order.time =  s.period.latest_trade_time || s.period.close_time
        ...
        if (signal === 'buy') {
          ...
          if (!size) {
            if (so.mode === 'live' || so.mode === 'paper') {
              ...
              var fee = so.order_type === 'maker' ? s.exchange.makerFee : s.exchange.takerFee
              var trade_balance = n(s.balance.currency).divide(100).multiply(buy_pct)
              var tradeable_balance = n(s.balance.currency).divide(100 + fee).multiply(buy_pct)
              var expected_fee = n(trade_balance).subtract(tradeable_balance).format('0.00000000', Math.ceil) // round up as the exchange will too
              if (buy_pct + fee < 100) {
