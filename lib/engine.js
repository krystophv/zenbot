...
module.exports = function container (get, set, clear) {
  ...
  return function (s) {
    ...
    function placeOrder (type, opts, cb) {
      ...
      if (so.mode !== 'live') {
        if (!order.orig_time) order.orig_time = s.period.latest_trade_time || s.period.close
        order.time = s.period.latest_trade_time || s.period.close
    ...
    function onPeriodCallback() {
      ...
      if (!s.in_preroll) {
        ...
        if (so.mode !== 'live') {
          var trade = {
            time: tb(s.period._id).resize(so.period_length).add(1).toMilliseconds() -1
          }
          if (s.buy_order) {
    ...
    return {
      ...
      updatePeriods: function (periods, is_preroll, cb) {
        ...
        while( (period = local_periods.shift()) !== undefined ) {
          period.id = tb(period._id).resize(so.period_length).toString()
