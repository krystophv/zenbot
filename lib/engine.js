...
module.exports = function (s, conf) {
  ...
  function executeSignal (signal, _cb, size, is_reorder, is_taker) {
    ...
    syncBalance(function (err) {
      ...
      getQuote(function (err, quote) {
        let reorder_pct, trade_balance, tradeable_balance, expected_fee, fee
        if (err) {
  ...
  function onPeriodCallback(cb) {
    if (!clock && so.mode !== 'live') clock = lolex.install({ shouldAdvanceTime: false, now: s.period.close_time })
    ...
    if (!s.in_preroll) {
      if (so.mode !== 'live'){
        if (s.buy_order) {
          ...
          trade.size = s.buy_order.size
        } 
        if (s.sell_order) {
          trade.size = s.sell_order.size
