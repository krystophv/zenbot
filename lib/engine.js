...
module.exports = function (s, conf) {
  ...
  eventBus.on('periods', onPeriods)
  eventBus.on('period', queuePeriod)
    ...
    if (s.my_trades.length || s.my_prev_trades.length) {
      ...
      if (!s.acted_on_stop) {
        if (last_trade.type === 'buy') {
          ...
          if (so.profit_stop_enable_pct && s.last_trade_worth >= (so.profit_stop_enable_pct / 100)) {
          ...
          if (do_sell_stop && s.profit_stop && s.period.close < s.profit_stop && s.last_trade_worth > 0) {
            stop_signal = 'sell'
  ...
  function executeSignal (signal, _cb, size, is_reorder, is_taker) {
    ...
    syncBalance(function (err) {
      ...
      getQuote(function (err, quote) {
        let reorder_pct, trade_balance, tradeable_balance, expected_fee
        if (err) {
        ...
        if (signal === 'buy') {
          ...
          if (!size) {
            ...
            if (so.use_fee_asset) {
              ...
              s.buy_order.fee = s.exchange.makerFee
            } else {
              s.buy_order.fee = s.exchange.takerFee
            }
            trade_balance = n(s.balance.currency).divide(100).multiply(buy_pct)
            tradeable_balance = n(s.balance.currency).divide(100 + s.buy_order.fee).multiply(buy_pct)
            expected_fee = n(trade_balance).subtract(tradeable_balance).format('0.00000000', Math.ceil) // round up as the exchange will too
            if (buy_pct + s.buy_order.fee < 100) {
          ...
          if ((s.product.min_size && Number(size) >= Number(s.product.min_size)) || 
            ('min_total' in s.product && s.product.min_total && n(size).multiply(price).value() >= Number(s.product.min_total))) {
            if (s.product.max_size && Number(size) > Number(s.product.max_size)) {
              size = s.product.max_size
            }
            msg('preparing buy order over ' + fa(size) + ' of ' + fc(tradeable_balance) + ' (' + buy_pct + '%) tradeable balance with a expected fee of ' + fc(expected_fee) + ' (' + s.buy_order.fee + '%)')
  ...
  function writeReport (is_progress, blink_off) {
    ...
    process.stdout.write(moment(is_progress ? (s.period.latest_trade_time || s.period.close_time) : tb(s.period.time).resize(so.period_length).add(1).toMilliseconds()).format('YYYY-MM-DD HH:mm:ss')[is_progress && !blink_off ? 'bgBlue' : 'grey'])
  ...
  function onPeriodCallback(cb) {
    ...
    if (!s.in_preroll) {
      ...
      if (so.mode !== 'live'){
        if (s.buy_order) {
          trade.price = s.buy_order.price
        } 
        if (s.sell_order) {
          trade.price = s.sell_order.price
        }
        s.exchange.processTrade(trade)
      }
      if (so.mode !== 'live' && !s.start_capital) {
        s.start_capital = 0
        s.start_price = s.period.close
        if (so.asset_capital) {
          s.start_capital += so.asset_capital * s.start_price
        }
        if (so.currency_capital) {
  ...
  var q2 = async.queue(function(period, callback){
    onPeriod(period, null, callback)
  })

  function queuePeriod(period){
    q2.push(period)
  }
  ...
  function onPeriod(period, is_preroll, cb) {
    var period_bucket = tb(period._id).resize(so.period_length)
    period.time = period._id
    period.period_id = period_bucket.toString()
    period.close_time = period_bucket.add(1).toMilliseconds() - 1
    var day = (new Date(period._id)).getDate()
    if (s.last_day && s.last_day && day !== s.last_day) {
      s.day_count++
    }
    s.last_day = day
    s.in_preroll = is_preroll || (so.start && period._id < so.start)
    s.period = period
    s.strategy.calculate(s)
  ...
  function onPeriods(periods, is_preroll, cb) {
    if (_.isFunction(is_preroll)) {
      cb = is_preroll
      is_preroll = false
    }
    var local_periods = periods.slice(0)
      
    var period
    while( (period = local_periods.shift()) !== undefined ) {
      onPeriod(period, is_preroll)
    }
    if(_.isFunction(cb)) cb()
  }
